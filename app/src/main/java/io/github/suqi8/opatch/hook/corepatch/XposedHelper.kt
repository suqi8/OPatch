...
open class XposedHelper {
    ...
    fun hookAllMethods(hookClass: Class<*>?, methodName: String?, callback: XC_MethodHook?) {
        try {
            ...
            if (BuildConfig.DEBUG) XposedBridge.log(
                "E/" + MainHook.TAG + " " + Log.getStackTraceString(
                    e
                )
            )
        }
    ...
    companion object {
        ...
        fun hookAllMethods(
            ...
            try {
                val packageParser = findClass(className, classLoader)
        ...
        fun hookAllConstructors(className: String?, callback: XC_MethodHook?) {
            try {
                val packageParser = findClass(className, null)
